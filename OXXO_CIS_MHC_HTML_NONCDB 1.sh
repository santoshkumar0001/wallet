#!/usr/bin/env bash
# =============================================================================
# OXXO Oracle CIS / MHC HTML Assessment (Non-CDB Version)
# Oracle Database 19c Non-CDB configuration inventory
#
# Usage:
#   ./OXXO_CIS_MHC_HTML_NONCDB.sh <ORACLE_SID> <ORACLE_HOME> [output_directory]
#
# Example:
#   ./OXXO_CIS_MHC_HTML_NONCDB.sh EBSDBQA /u01/oracle/OICEBSQA/19.1.0 /home/oracle/MHC
#
...
<header>
  ...
  <p>Oracle Database 19c configuration evidence report (Non-CDB)</p>
</header>
<main>
...
<section class="notice">
  <strong>Scope and evidence.</strong> This report shows the live configuration values retrieved at assessment time for a Non-CDB database environment. SQL text is displayed beneath every database result set. Only the <code>DEFAULT</code> password profile is assessed.
</section>
EOF
}
...
write_html_footer() {
  cat >> "$REPORT" <<'EOF'
<footer>
  Generated by OXXO_CIS_MHC_HTML_NONCDB.sh. This report is an evidence inventory; a <strong>REVIEW</strong> result requires validation against the approved exception register and the applicable CIS benchmark.
</footer>
</main>
</body>
</html>
...
run_sql_fragment() {
  local title="$1" statement="$2" rc
  printf '<h3>%s</h3>\n' "$(html_text "$title")" >> "$REPORT"
  cat >> "$REPORT" <<EOF
<details><summary>View executed SQL</summary><pre>$(printf '%s' "$statement" | html_escape)</pre></details>
...
if ! DB_CHECK="$("$SQLPLUS" -s -L '/ as sysdba' <<'SQL'
whenever oserror exit failure rollback
whenever sqlerror exit sql.sqlcode rollback
...
if [[ "$DB_CHECK" == "YES" ]]; then
  echo "WARNING: Connected database reports CDB=YES. Standard non-CDB script executing." >&2
fi

cat >> "$REPORT" <<'EOF'
<h2>1. Database Scope</h2>
<p class="section-note">The following query establishes the target database identity.</p>
EOF
...
run_sql_fragment "Database identity" "$DATABASE_ID_SQL"
...
cat >> "$REPORT" <<'EOF'
<h2>3. Initialization Parameter and Default Profile Evidence</h2>
<p class="section-note">This query checks effective initialization parameters and profile defaults.</p>
EOF

PARAMETER_SQL=$(cat <<'SQL'
with controls (section, parameter, expected_value) as (
  ...
  select inst_id, upper(name) parameter, nvl(value,'<NULL>') actual_value
...
order by c.section, p.inst_id nulls last;
SQL
)
run_sql_fragment "CIS 2.3 initialization parameters" "$PARAMETER_SQL"
...
run_sql_fragment "CIS Section 3 - DEFAULT profile only" "$DEFAULT_PROFILE_SQL"
...
run_sql_fragment "CIS 3.3 - DEFAULT profile life and grace values" "$PROFILE_TOTAL_SQL"

cat >> "$REPORT" <<'EOF'
<h2>4. Users, Password Files, and Audit Configuration</h2>
<p class="section-note">These queries audit user security settings, database links, password files, and unified auditing policies.</p>
EOF
...
from dba_users
order by section, database_name, username;
SQL
)
run_sql_fragment "CIS 4.1, 4.2, and 4.4 - user inventory" "$USER_SECURITY_SQL"
...
run_sql_fragment "CIS 4.7 and 4.8 - database-link inventory" "$DBLINK_SQL"
...
run_sql_fragment "CIS 4.5 and 4.6 - password-file inventory" "$PASSWORD_FILE_SQL"
...
order by section, p.policy_name, p.audit_option;
SQL
)
run_sql_fragment "CIS Section 5 - unified audit policy inventory" "$AUDIT_SQL"

cat >> "$REPORT" <<'EOF'
<h2>5. Privileged Access Evidence</h2>
<p class="section-note">All identified grants are presented for validation against the approved access-exception register.</p>
EOF
...
select m.section,
       ...
       'ADMIN_OPTION='||s.admin_option as actual_value,
...
run_sql_fragment "CIS 6.1 - system privilege inventory" "$SYSTEM_PRIV_SQL"
...
select m.section,
       ...
       r.grantee,
       r.granted_role,
       'ADMIN_OPTION='||r.admin_option as actual_value,
       'No unauthorized grants; validate approved exceptions' as expected_value,
       'REVIEW' as compliance
from section_map m
...
run_sql_fragment "CIS 6.2 - privileged role inventory" "$ROLE_PRIV_SQL"
...
run_sql_fragment "CIS 6.3.1 - sensitive SYS-table privileges" "$SENSITIVE_TABLE_SQL"
